""" 
This is the main file to train the model using MLflow.
"""
import pathlib
import splitfolders
import torch
...
from src.data_propessing.datapipe import KaggleDataPipe
...
# # uncomment these lines only for the first run
...
class TumorAnalysisModel(object):
    ...
    3) Train the model
    """
    def __init__(self, mode="test", verbose=False):
        self.mode = mode
    ...
    def load_data(self, load_params):
        ...
        """
        mlflow.log_param("load_params",load_params)
    ...
    def train(self, train_params,verbose=False):
        ...
        mlflow.log_param("train_params",
                        {"epochs":train_params["epochs"],
                         "lr": train_params["lr"],
                         "optimiser":train_params["optimiser"].state_dict(),
        ...
        for epoch in tqdm(range(epochs)):
            ...
            mlflow.log_metric("train_accuracy", f"{train_metric:2f}", step=epoch)           
...
@click.option(
    "--mode",
    type = click.Choice(["test","dev"],case_sensitive=False),
    required = True,
    help = "Specify the mode (either 'test' or 'dev')"
)
def main(mode):
    from src.utils.utils import CLA_label
    workflow = TumorAnalysisModel(mode, verbose=True)
    with mlflow.start_run() as run:
        ...
        train_params = {
            ...
            "weight_path": "model_under_training.pt",
        }
        workflow.train(train_params)
...
if __name__ == "__main__":
    main()
        ...
        #         f.write(str(summary(workflow.model)))
        # mlflow.log_artifact("model_summary.txt")
        # workflow.train(train_params, verbose=True)
        # mlflow.pytorch.log_model(workflow.model, "model")
